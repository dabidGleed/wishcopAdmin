...
app.directive("starRating", function($compile, $timeout, stars, starsUtility) {
    return {
        ...
        template: '<div class="stars" ng-mousemove="changeRating($event)" ng-mouseleave="leaveRating()"><div class="stars-selected addReviewStar" ng-style="{\'width\': percent + \'%\', \'background-color\': selColor}"></div></div>',
        link: function($scope, el, attrs) {
            ...
            $scope.ratingDefine && ($scope.$watch("percent", function(newValue) {
                ...
                var star = angular.element('<canvas class="star" ng-style="{\'background-color\': emptyBackColor}" ng-click="secureNewRating()" height="{{starRadius*2}}" width="{{starRadius*2}}"></canvas>');
